...
function getStage(percent: number) {
  ...
  if (p < 25) return { idx: 1, total: 4, msg: '사업장 정보를 분석하고 있어요' };
  if (p < 50) return { idx: 2, total: 4, msg: '세부 공정 데이터를 분석하고 있어요' };
  if (p < 75)
...
export default function ProgressDownloadModal({
  ...
  // ✅ Portal이므로 document가 없을 수 있는 환경 대비
