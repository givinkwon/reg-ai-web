...
export const extractSafetyArticlesHtml = (html: string) => {
  ...
  // display:none 같은 스타일이 들어오면 제거(선택)
  const cleaned = match[0].replace(/display\s*:\s*none\s*;?/gi, '');

  // 기존 RightPanel에서 하던 것처럼 헤더를 붙이고 싶으면:
  return `<div>
    <h3 style="margin:0 0 10px; font-size:14px; font-weight:800;">참고 기사 목록</h3>
    ${cleaned}
  </div>`;
};

/** digest 텍스트를 (요약/기사)로 나누는 fallback */
