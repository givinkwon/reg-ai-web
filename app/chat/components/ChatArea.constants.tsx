...
import { Search, AlertTriangle, FileText, Folder } from 'lucide-react';
...
export const TASK_META: Record<TaskType, { label: string }> = {
...
export const QUICK_ACTIONS: QuickAction[] = [
  ...
  },
  {
    ...
    icon: FileText,
    placeholder:
      '우리 사업장(작업, 규모, 주요 공정)에 맞는 안전보건 실무지침을 정리해줘.',
...
export const makeDownloads = (id: string, baseLabel: string): SafetyDocDownload[] => [
  {
...
export const QUICK_ACTIONS_MAP: Record<string, QuickAction> = QUICK_ACTIONS.reduce(
  (acc, cur) => {
    acc[cur.id] = cur;
    return acc;
  },
...
export const GUEST_LIMIT_COOKIE_KEY = 'regai_guest_msg_count';
  ...
  export const LAW_INTERPRET_HINTS: string[] = [
    '우리 사업장의 작업, 인원, 주요 공정을 알려줄테니 기본적으로 지켜야 할 안전보건 의무를 정리해줘.',
    ...
    '최근 개정된 중대재해처벌법이 우리 작업에 어떤 의무를 추가로 요구하는지 알려줘.',
